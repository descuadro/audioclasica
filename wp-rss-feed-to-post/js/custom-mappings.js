	...
	$(window).load( function(){
		...
		$('#wprss-ftp-namespace-detector-refresh').click( function() {
			...
			if ( url !== '' ) {
				...
				$('#wprss-ftp-namespace-detector-results').text("Please wait ...");
				...
				$.ajax({
					...
					complete: function( jqXHR, status ) {
						...
						try {
    						...
    						for( i in obj ) {
    							$('<li><i class="fa fa-check"></i> Namespace <code>'+i+'</code> with URL: <code>'+obj[i]+'</code></li>').appendTo( list );
    						}
    					}
			...
			// Else, if not URL has been entered
				...
				$('#wprss-ftp-namespace-detector-results').text('Please specify a feed source URL first.');
			}
		});
	});
