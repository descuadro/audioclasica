...
/*
Copyright 2013-2014 Jean Galea (email : info@jeangalea.com)
...
if( !defined( 'WPRSS_FTP_VERSION' ) )
	define( 'WPRSS_FTP_VERSION', '3.0', true );
...
if ( !defined( 'WPRSS_FTP_SL_STORE_URL' ) ) {
	define( 'WPRSS_FTP_SL_STORE_URL', 'http://www.wprssaggregator.com' ); 
}
...
final class WPRSS_FTP {
	 ...
	 * Dependancy versions.
	 */
	const WP_MIN_VERSION = '3.7';
	const WPRSS_MIN_VERSION = '4.0.2';
	...
	public function override_shortcode( $output ) {
		if ( wprss_ftp_using_feed_items() === TRUE )
			...
			?>
				<p>
					The Feed to Post add-on disables the shortcode functionality by default.
					<a href="http://www.wprssaggregator.com/docs/add-compatibility" target="_blank">Read why here</a>
					and
					<a href="http://www.wprssaggregator.com/docs/using-the-shortcodes " target="_blank">how to re-enable it here</a>.
				</p>
			<?php
			endif;
			return ob_get_clean();
	...
	/**
	 * Checks if the license has been entered. If not, adds an action for a notification.
	 * 
	 * @since 1.3
	 */
	public function check_license(){
        ...
        $license_key = ( isset( $license_keys['ftp_license_key'] ) ) ? $license_keys['ftp_license_key'] : '';
        $option = wprss_check_addon_notice_option();
		if ( strlen( $license_key ) === 0 && isset( $option['feed_to_post']['license'] ) === FALSE && is_main_site() ) {
            add_action( 'all_admin_notices', array( $this, 'license_notification' ) );
        }
	}
	...
	public function notify_about_disabled_addons() {
		...
		<div class="updated">
			<p>
				...
				<a href="http://www.wprssaggregator.com/docs/add-compatibility" target="_blank">Read why here</a>
				and
				<a href="http://www.wprssaggregator.com/docs/using-the-shortcodes " target="_blank">how to re-enable them here</a>.
			</p>
		</div>
		<?php
	}
	...
	public function enqueue_scripts_and_styles() {
		...
		wp_enqueue_script( 'wprss_ftp_admin_scripts', WPRSS_FTP_JS . 'admin-scripts.js' );

		// If in an edit page
		if ( $screen->base === 'post' || $screen->base === 'edit' ) {
			// If editing a feed source
	...
	public function add_wprss_feed_columns( $columns ) {
		$columns['post-type'] = __( "Post Type", "wprss" );
		return $columns;
	}
	...
	public function render_wprss_feed_columns( $column, $post_id ) {
		switch ( $column ) {
			case 'post-type':
				...
				echo sprintf('<p><span class="%3$s">%1$s</span> (<span class="wprss-post-status-name">%2$s</span>)</p>', $label, $post_status_name, implode(' ', $post_type_class));
				break;
		}
	}
	...
	*/
	public function license_notification() {
		echo '<div class="updated">
			<p>Remember to <a href="' . admin_url( 'edit.php?post_type=wprss_feed&page=wprss-aggregator-settings&tab=licenses_settings' ) . '">enter your plugin license code</a>
				for the WP RSS Aggregator <b>Feed to Post</b> add-on, to benefit from updates and support. 
				<a href="#" class="ajax-close-addon-notice" style="float:right;" data-addon="feed_to_post" data-notice="license">Dismiss this notification</a>
			</p>
		</div>';
	}
	 ...
	 * @since 2.9
	 */
	public function view_feed_items_row_action_link( $link ) {
		$id = get_the_ID();
	...
	public function delete_posts_row_action_title( $title ) {
		return 'Delete posts imported by this feed source';
	}
	...
	public function export_section() {
		?>
		<p><?php _e( '<strong>Note:</strong> Your <em>license codes</em> and your Category <em>Post Terms</em> options for Feed to Post add-on are not exported.', 'wprss' ); ?></p>
		<?php
	}
	...
	public static function wp_dependancy_msg() {
		return __( 'The Feed to Post add-on requires WordPress version ', 'wprss' ).self::WP_MIN_VERSION.' '.__( 'or higher', 'wprss' ) . '.';
	}
	...
	public static function wprss_dependancy_msg() {
		return __( 'Please install and activate WP RSS Aggregator, at version', 'wprss' ) . ' <b>'.self::WPRSS_MIN_VERSION.'</b> ' . __( 'or higher, for the Feed to Post add-on to work.', 'wprss' );
	}
	...
	public static function disabled_addons_msg() {
		return __( 'The following WP RSS Aggregator add-ons have been disabled by the Feed-to-Post add-on', 'wprss' );
	}
	...
	public static function wprss_ftp_deleting_source_posts_msg() {
		return __( 'The posts for the selected feed source are being deleted in the background.', 'wprss' );
	}
