...
class WPRSS_FTP_Url_Shortener {
	...
	public static function register_settings($settings) {
		...
		add_settings_section(   
			...
			__( 'URL Shortening', 'wprss' ),			    // Title to be displayed on the administration page
		...
		add_settings_field(
			...
			__( 'Method', 'wprss' ),				// The label to the left of the option element
		...
		add_settings_field(
			...
			__( 'Google API Key', 'wprss' ),			// The label to the left of the option element
	...
	public static function render_url_shortening_section() {
	    ?><p><?php _e('Configure URL Shortening', 'wprss') ?></p><?php
	}
	...
	public static function render_google_api_key($args) {
		...
		<p class="description"><?php echo sprintf(__('This key will be used for requests to the %1$sGoogle URL Shortener API%2$s'), '<a href="https://developers.google.com/url-shortener/v1/getting_started">', '</a>') ?></p>
		<?php
	}
	...
	public static function add_meta_fields($fields) {
		$fields['url_shortening'] =	array(
			...
			self::META_KEY_URL_SHORTENING_METHOD	=> array(
				'label'			=> __('Method'),
				'desc'			=> __('The service or algorithm to use for shortening'),
				'type'			=> 'dropdown',
	...
	public static function add_meta_boxes() {
		...
		add_meta_box(
			...
			__( 'Feed to Post - URL Shortening', 'wprss' ),		    // $title 
	 ...
	 */
	public static function render_url_shortening_metabox($post) {
		WPRSS_FTP_Meta::get_instance()->render_metabox('url_shortening', FALSE);
	}
	...
	public static function add_shortening_method_google_url_shortener_api($methods) {
		...
		$methods['google_url_shortener_api'] = __('Google URL Shortener API');
		return $methods;
	}
