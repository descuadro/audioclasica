...
final class WPRSS_FTP_Debug {
	...
	public function __construct() {
		if ( self::$instance === NULL ) {
			...
			wp_die("WPRSS_FTP_Debug class is a singleton class and cannot be redeclared.");
		}
	}
	...
	public function debug_deleting_posts_notice() {        
        echo '<div class="updated"><p>All posts created by the Feed To Post add-on are being deleted in the background.</p></div>';
    }
	...
	public function debugging_delete_all_posts() {
		?>
		<h3><?php _e( 'Delete imported posts (Feed To Post)', 'wprss' ); ?></h3>
		<p><?php _e( 'Click the red button to delete all posts imported by the Feed to Post add-on.', 'wprss' ); ?></p>
		<p><?php _e( '<em><strong>Note:</strong> This is a server-intensive process and can take long, depending on how much posts you have imported.</em>', 'wprss' ); ?></p>            
	...
	public function show_error_log() {
		?>
		<h3><?php _e( 'Feed To Post Error Log', 'wprss' ); ?></h3>

		<textarea readonly="readonly" id="wprss-ftp-error-log-textarea"><?php echo WPRSS_FTP_Utils::get_log(); ?></textarea>

		<form action="edit.php?post_type=wprss_feed&page=wprss-debugging" method="POST"> 
			<?php wp_nonce_field( 'wprss-ftp-clear-error-log' );
			submit_button( __( 'Clear log', 'wprss' ), 'button-primary', 'ftp-error-log', true  ); ?>
		</form>
	...
	public function before_delete_post( $post_id ) {
		$meta = get_post_meta( $post_id, 'wprss_item_permalink', FALSE );
	...
	public function any_post_type( $where = '' ) {
		return preg_replace( '/(AND)? ([^\.]*\.)?post_type\s=\s[\"\']ignore[\"\']/', '', $where );
	}
