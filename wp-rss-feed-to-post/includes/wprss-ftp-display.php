	...
	public static function post_content( $content ) {
		if ( !is_feed() && is_singular() ) {
			...
			if ( $source !== '' ) {
				...
				if ( $prepend !== '' ) {
					$content = WPRSS_FTP_Appender::handle_appended_data( $post, $prepend ) . $content;
				}
				// Add any append text
				if ( $append !== '' ) {
					$content .= WPRSS_FTP_Appender::handle_appended_data( $post, $append );
				}
				

				// If the post's feed source has source_link enabled ...
				if ( WPRSS_FTP_Utils::multiboolean( $options['source_link'] ) === TRUE ) {
					// Prepare to data
					...
					if ( $linked_text !== '' ) {
                        $linkMethod = apply_filters('wprss_ftp_add_source_link_method', array(get_called_class(), 'add_source_link'));
	...
	public static function post_head() {
		...
		if ( is_singular() ) {
			...
			if ( $source !== '' ) {
				...
				$canonical_link = WPRSS_FTP_Settings::get_instance()->get( 'canonical_link' );
